...
onMounted(async () => {
  ...
  notas.value = all.filter((n) => n.tipo === 'NFC-e')
  loading.value = false
})
...
<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-xl font-semibold text-ink">NFC-e</h1>
      <p class="text-sm text-ink-soft mt-1">Notas fiscais de consumidor eletrônicas emitidas no PDV.</p>
    </div>
    ...
    <Card :padded="false" class="p-5 sm:p-6">
      ...
      <Table :columns="columns" :rows="paginated" :loading="loading" empty-icon="Receipt" empty-title="Nenhuma NFC-e emitida ainda">
